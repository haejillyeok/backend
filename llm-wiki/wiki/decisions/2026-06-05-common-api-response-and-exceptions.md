...
- 각 `ErrorDefinition`은 기본 message, HTTP status, gRPC status, WebSocket close code를 함께 가진다.
...
- `AppException`은 `ErrorCode`를 받으면 catalog에서 `message`, `http_status_code`, `grpc_status_code`, `websocket_close_code`, `error_type` 기본값을 가져온다.
