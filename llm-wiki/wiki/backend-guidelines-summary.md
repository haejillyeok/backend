...
- 서버 간 내부 통신은 gRPC 계약과 client wrapper로 수행한다.
- 한 서버가 다른 서버의 service/repository를 직접 import하지 않는다.
- outbound RPC에는 deadline을 명시한다.
- handler는 domain exception을 gRPC status로 변환한다.
- health check와 graceful shutdown을 유지한다.

## WebSocket

- 사용자-facing 실시간 통신은 FastAPI WebSocket을 기본으로 한다.
- 서버 간 단순 호출은 WebSocket이 아니라 gRPC를 우선한다.
...
- generated proto Python 파일은 직접 수정하지 않는다.
- 로직 설명과 함수 docstring은 한국어로 작성한다.
- public 함수, service/repository 메서드, gRPC handler, WebSocket connection manager 메서드는 의도, 입력, 반환값, 부작용을 설명한다.
